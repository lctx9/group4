...
3. **Agentic Exploration (điểm mới):** Mô hình tác nhân tự do khám phá repository thông qua terminal — gọi `pytest`, đọc execution log, tự sửa test suite theo vòng lặp phản hồi thực thi (Execution Feedback Loop), tối đa 80 turns. Mỗi tác vụ chỉ chạy duy nhất $k=1$ lượt mẫu (Pass@1 Budget).
4. **Code Action Fixing:** Mô hình Code Action Fixer — kích hoạt cơ chế suy luận chuyên sâu, nhận log lỗi cuối cùng từ Agent, sửa mã kiểm thử cụ thể để chốt kết quả đầu ra.
...
6. **Statistical analysis:** Wilcoxon signed-rank test so sánh cấu hình Agentic ($k=1$) vs. Plain context baseline ($k=1$) trên cùng tập dữ liệu tổng.
...
- Model: `DeepSeek-V3` (`deepseek/deepseek-chat` qua OpenRouter API)
- Hyperparameters: `temperature=0.0`, `max_tokens=4096`, `max_turns=80`, `k=1` (Pass@1)
- Prompting strategy: Zero-shot với system prompt định hướng Proactive Discovery
- Framework: SWEAgent — tương tác terminal thực (pytest, python interpreter)
- Lý do cấu hình: `DeepSeek-V3` có khả năng xử lý Tool-use và lệnh CLI terminal thực thi vượt trội; temperature=0.0 để đảm bảo tính ổn định suy luận.

**Code Action Fixer:**
- Model: `Llama-3.3-70B-Instruct` (`meta-llama/llama-3.3-70b-instruct` qua OpenRouter API)
- Hyperparameters: `temperature=0.0`, `max_tokens=4096`, `k=1`
- Prompting strategy: Few-shot với ví dụ bóc tách log lỗi và tổng hợp test suite
- Lý do cấu hình: `Llama-3.3-70B-Instruct` có năng lực suy luận mã nguồn chuyên sâu, tối ưu hóa tuyệt đối khả năng tổng hợp log lỗi từ terminal để xuất ra mã kiểm thử Python hoàn chỉnh.
...
Với temperature=0.0 cho Exploration Agent, các lần chạy có thể cho kết quả F2P khác nhau đáng kể — một lần cao, một lần thấp — làm khó kết luận.
