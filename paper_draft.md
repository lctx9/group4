...
### Bản Tiếng Việt (Giọng người dùng tự nhiên - 5 câu):
Viết test thủ công vẫn là một nút thắt cổ chai lớn trong kỹ nghệ phần mềm, thường chiếm tới một nửa tổng ngân sách phát triển của dự án. Các mô hình ngôn ngữ sinh code hiện nay hay mắc bẫy tuân thủ (compliance bias) khi nhắm mắt chấp nhận code bị lỗi, nhưng chưa có nghiên cứu nào thử nghiệm xem liệu phản hồi thực thi terminal có thể chặn được lỗi này hay không. Chúng tôi xây dựng một hệ thống hai agent mã nguồn mở nối DeepSeek-V3 trong terminal SWE-agent CLI thực thời gian thực với Llama-3.3-70B, và chạy thử nghiệm trên toàn bộ 1,552 tác vụ Python của TestExplora. Phản hồi thực thi terminal giúp đạt tỉ lệ Fail-to-Pass 35.05% ở Pass@1 so với 16.60% của baseline tĩnh, đạt tới 71.59% ở Pass@3 với p = 0.000000 và Cliff's delta = 0.3018. Kết quả này cho thấy việc cấp quyền truy cập command line trực tiếp cho LLM giúp phát hiện lỗi thực tế trước khi code được đẩy lên môi trường thật.

### English Abstract (Ultra-Natural Human Researcher Style - 5 Sentences):
