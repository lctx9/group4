...
\begin{table}[htbp]
\caption{STATISTICAL DISPERSION AND VARIANCE ANALYSIS ACROSS SAMPLING BUDGETS}
\begin{center}
\begin{tabular}{|l|c|c|c|c|c|}
\hline
\textbf{Sampling Metric} & \textbf{Mean} & \textbf{Median} & \textbf{Std Dev} & \textbf{IQR (25\% - 75\%)} & \textbf{Range [Min, Max]} \\
\hline
Pass@1 (Across Runs) & 35.05\% & 34.86\% & 0.93\% & [34.44\%, 35.57\%] & [34.02\%, 36.28\%] \\
Pass@3 (Per Task) & 71.59\% & 100.0\% & 45.10\% & [0.00\%, 100.0\%] & [0.00\%, 100.0\%] \\
Baseline F2P (Run 1-3) & 16.60\% & 17.07\% & 1.59\% & [14.82\%, 17.91\%] & [14.82\%, 17.91\%] \\
\hline
\end{tabular}
...
\begin{table}[htbp]
\caption{FORMAL HYPOTHESIS SIGNIFICANCE & DECISION MATRIX}
\begin{center}
\begin{tabular}{|c|l|c|c|c|c|}
\hline
\textbf{RQ} & \textbf{Hypothesis Statement} & \textbf{Wilcoxon $W$} & \textbf{$p$-value} & \textbf{Cliff's $\delta$} & \textbf{Decision} \\
\hline
RQ1 & $H_0: \text{Pass@1} \le 16.06\%$ & 239,987.5 & 0.000000 & 0.3018 & \textbf{Reject $H_0$} \\
RQ2 & $H_0: \text{Pass@3} \le \text{Pass@1}$ & N/A & $< 1.0\times 10^{-5}$ & 0.7412 & \textbf{Reject $H_0$} \\
\hline
\end{tabular}
...
\begin{table}[htbp]
\caption{QUALITATIVE FAILURE MODE CATEGORIZATION ($N=441$ UNSOLVED TASKS)}
\begin{center}
\begin{tabular}{|l|c|c|l|}
\hline
\textbf{Failure Category} & \textbf{Tasks} & \textbf{\%} & \textbf{Representative Diagnostic Traceback} \\
\hline
Complex Env Dep. & 282 & 18.20\% & \texttt{ImportError: C extension dynamic load failed} \\
Turn Exhaustion & 159 & 10.21\% & \texttt{MaxTurnsExceeded: Reached 80 step ceiling} \\
\hline
\textbf{Total Unsolved} & \textbf{441} & \textbf{28.41\%} & \textit{Out of N=1,552 tasks at Pass@3} \\
\hline
\end{tabular}
\label{tab:failures}
\end{center}
...
\caption{Comparison of Average Pass@1 F2P rate against baseline.}
...
\caption{Pass@k sample budget scaling effect (Pass@1, Pass@2, Pass@3).}
\label{fig:scaling}
\end{figure}
...
\caption{Performance consistency and variance across 3 independent runs (K=3).}
